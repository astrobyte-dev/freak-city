...
def review_page(batch, output):
    ...
    for index,(path,meta) in enumerate(candidates,1):
        ...
        parts.append(f'<article><h2>{index:02} / seed {meta["seed"]}'+(f' / denoise {settings["strength"]}' if "strength" in settings else '')+'</h2><div class="grid">')
        ...
        parts.append('</div>')
        for key,label in (("sourceImage","Retained source"),("pixelSource","Pixel master")):
        ...
        if role=="canonical-room":
            ...
            parts.append('<details open><summary>Canonical facts and routes</summary><ul>'+''.join(f'<li>{text(f["text"])}</li>' for f in facts)+'</ul><p>Routes: '+text(', '.join(e['to'] for e in meta['reviewContract']['exits']))+'</p></details>')
        ...
        parts.append('<details><summary>Review criteria and provenance</summary><pre>'+text(json.dumps({k:meta.get(k) for k in ("requiredFacts","forbiddenFacts","conditioning","structurePreservation","provenance")},indent=2))+'</pre></details></article>')
    parts.append('</html>')
