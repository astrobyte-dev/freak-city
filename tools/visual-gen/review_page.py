...
def review_page(batch, output):
    ...
    for index,(path,meta) in enumerate(candidates,1):
        ...
        parts.append('</div>')
        for key,label in (("sourceImage","Retained source"),("pixelSource","Pixel master"),("uncompositedModelOutput","Model output before hard mask restoration")):
        ...
        parts.append('<details><summary>Review criteria and provenance</summary><pre>'+text(json.dumps({k:meta.get(k) for k in ("generationSettings","environment","requiredFacts","forbiddenFacts","conditioning","structurePreservation","provenance")},indent=2))+'</pre></details></article>')
    parts.append('</html>')
