...
export function describeTrial(s: TrialState) {
  ...
  return [
    ...
    ...available.map((n) => `Here: ${n}.`),
    ...(s.drink &&
    (s.drink.location === s.room || s.drink.location === "player")
      ? [
          `Your ${s.drink.remaining ? s.drink.kind : "empty glass"} is ${s.drink.location === "player" ? "in your hand" : "within reach on the counter"}.`,
        ]
      : []),
  ];
}
...
function respond(
  ...
  if (text === "take")
    return clarify(
      `What would you like to take? ${s.drink && s.drink.location === s.room ? `Your ${s.drink.kind} is within reach. ` : ""}Name a visible object; LOOK can help.`,
      "take:missing-object",
    );
  if (
    ...
    return {
      lines: [
        ...
        raw.trim().toLowerCase() === `${s.alias.toLowerCase()}?`
          ...
          : s.preference
            ? `Sable remembers that you asked for ${s.preference}. ‘Usual’ means that preference, not a different kind of drink. ${s.drink ? "You can check the drink actually served with LOOK." : "Nothing has been served yet in this recorded state."}`
  ...
  if (/^(inventory|belongings|what am i carrying)$/.test(text))
    return {
      lines: Object.values(s.entities)
        ...
        .concat(
          s.drink?.location === "player"
            ? [`Your ${s.drink.remaining ? s.drink.kind : "empty glass"}`]
            : [],
  ...
  if (
    /^(sip|drink|finish|take|pick up)(?: (?:the|my|some|a sip of))? (water|tea|coffee|drink|glass|cup|special|cocktail|minor administrative disappointment)$/.test(
      text,
    )
  ) {
    const drink = s.drink;
    if (!drink || ![s.room, "player"].includes(drink.location))
      return no(
        "There isn't a served drink within reach. Ask for one while Sable is here.",
      );
    const named =
      text.match(/water|tea|coffee/)?.[0] ??
      (/special|cocktail|minor administrative disappointment/.test(text)
        ? drink.kind.includes("special")
          ? drink.kind
          : "the special"
        : undefined);
    if (named && named !== drink.kind)
      return clarify(
        `The drink here is ${drink.kind}. Did you mean that, or do you want to order ${named}?`,
        "drink:wrong-drink",
      );
    if (/^(take|pick up)/.test(text)) {
      drink.location = "player";
      return {
        lines: [
          `You pick up your ${drink.remaining ? drink.kind : "empty glass"}.`,
        ],
        minutes: 1,
        intent: "drink:take",
      };
    ...
    if (!drink.remaining)
      return no("The glass is empty. You can ask for another drink.");
    drink.remaining = text.startsWith("sip") ? drink.remaining - 1 : 0;
    return {
      lines: [
        drink.remaining
          ? `You take a sip of ${drink.kind}.`
          : `You finish your ${drink.kind} and lower the empty ${drink.kind === "water" || drink.kind.includes("special") ? "glass" : "cup"}.`,
      ],
      minutes: 1,
      intent: "drink:consume",
    };
  }
  ...
  const order = drinkOrder(text);
  if (
    order &&
    !c.negated &&
    !/\b(?:don't|do not|can't|cannot|not|never)\b/.test(reply)
  ) {
    if (!sablePresent(s)) return no("Sable isn't here to take an order.");
    if (order.kinds.length !== 1 || order.uncertain)
      return clarify(
        "Which drink would you like: tea, coffee, water, or the special? The special is alcohol-free unless you ask for gin. Nothing has been served.",
        "drink:clarify-order",
      );
    const kind = order.kinds[0];
    record(
      s,
      "sable",
      "heard",
    ...
    return { ...ok(serveDrink(s, kind)), intent: "conversation:order-drink" };
  }
  if (
    /^no thank(?:s| you)\b/.test(reply) &&
    activeContext(s)?.question &&
    activeContext(s)?.topic === "drink"
  ) {
    if (/\b(but|instead|or)\b/.test(reply))
      return clarify(
        "Are you declining the drink, or asking for a different one? Nothing has been served.",
        "drink:clarify-refusal",
      );
    if (!sablePresent(s)) return no("Sable isn't here to hear that reply.");
    setTopic(s, "drink", "drink");
    return {
      ...ok("Sable: ‘Of course. Company doesn't have a minimum order.’"),
  ...
  if (
    ...
    if (
      context?.question &&
      /^(yes|no|no thanks|no thank you|that sounds good|i agree)$/.test(
        reply,
      ) &&
      context.topic === "drink"
    ) {
      ...
      if (context.question.kind === "confirm-drink" && context.question.offered)
        return {
          ...ok(serveDrink(s, context.question.offered)),
          intent: "conversation:accept-offered-drink",
        };
      return clarify(
        "Sable: ‘Which would you like: tea, coffee or water?’",
        "conversation:choose-drink",
      );
    }
    if (/\b(tea|coffee|water)\b/.test(reply)) {
      const drink = /coffee/.test(reply)
        ? "coffee"
        : /water/.test(reply)
          ? "water"
          : "tea";
  ...
  if (
    ...
    return clarify(
      "I haven't acted on that qualified statement. Are you declining something, or asking a question? Your current conversation is unchanged.",
      "conversation:qualified-statement",
    );
  const claim =
    /\b(?:i (?:found|saw|have|discovered|came across)|there is|there's)\b/.test(
      reply,
    ) && /\b(photo|photograph|picture|print)\b/.test(reply);
  if (
    (c.verb === "tell" && recipient === "sable" && !/tell me/.test(text)) ||
    (claim && s.room !== "shop")
  ) {
    if (!sablePresent(s))
      return no("Sable isn't present to receive that account.");
    const o = record(
      s,
      "sable",
      "claim",
    ...
    s.actors.sable.beliefs[o.id] = {
      text: raw.slice(0, 500),
      source: o.id,
      at: s.time,
    };
    setTopic(s, "evidence", "photo");
    return {
      ...ok(
        "Sable: ‘You found a photograph? I'd like to see it. Hearing about it isn't the same as inspecting it; let's keep the claim separate from what we can check.’",
      ),
      intent: "conversation:photo-claim",
    };
  }
  if (
    recipient === "vesper" &&
  ...
  const context = activeContext(s);
  const followup =
    /^(tell me more|what else do you remember|go on|and then|why|what do you mean|what do you think(?: (?:of|about) (?:it|that))?|what does (?:it|that) mean|how(?:'s| is) (?:it|that) going)$/.test(
      reply,
    );
  ...
  if (recipient === "sable" || topic || followup || socialOpening) {
    ...
    if (topic === "photo" || topic === "listing") {
      const evidence = topic === "photo" ? "trial-photo" : "trial-listing";
      if (!inspected(s, "sable", evidence)) {
        if (
          !s.actors.player.knowledge.includes("hospital-account") &&
          !s.observations.some((o) => o.actor === "sable" && o.mode === "claim")
        )
          return clarify(
            "Which photograph or date are you asking about? You haven't shown Sable an object or explained what you found.",
            "conversation:unintroduced-evidence",
          );
        setTopic(s, "evidence", topic);
        return {
          ...ok(
            `Sable: ‘I haven't seen the ${s.entities[evidence].name}. Can you show it to me? I can't judge its details from a question.’`,
          ),
          intent: "conversation:evidence-unseen",
        };
      }
      setTopic(s, "evidence", topic);
      return {
        ...ok(
          s.receipt
            ...
            : "Sable: ‘I can see the picture, but the occasion needs a separate dated source. The photograph alone doesn't settle it.’",
        ),
        intent: `conversation:${topic}`,
      };
    }
...
export function executeTrial(state: TrialState, raw: string): TrialState {
  ...
  for (const command of commands) {
    ...
    if (result.failed || (s.context?.kind === "decision" && !s.decision)) {
