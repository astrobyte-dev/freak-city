...
export const commandDiagnostic = z.object({
  command: z.string(),
  intent: z.string(),
  outcome: z.enum(["handled", "rejected", "clarified"]),
  from: instant,
  to: instant,
  roomBefore: roomId,
  roomAfter: roomId,
...
export const trialSchema = z.object({
  ...
  version: z.literal(1),
  revision: z.literal(2).default(2),
  ...
  drink: z
    .object({
      ...
      location: z.union([roomId, z.literal("player")]),
    })
    .optional(),
...
export function validateTrial(raw: unknown): TrialState {
  ...
  for (const [id, e] of Object.entries(s.entities)) {
    fail(
      e.id !== id || e.kind !== expected[id].kind,
  ...
  fail(!!s.drink && s.drink.servedAt > s.time, "Drink predates its service.");
  fail(
