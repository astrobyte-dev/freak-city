...
const common = [
  ...
  "no",
  "yes",
  "no it doesn't sound threatening",
  "a little",
  "sounds fine to me",
  "it sounds fine",
  "not really",
  ...
  "tell me about the prom",
  "tell me about the supplier",
  "what's wrong",
  "you seem tired",
  "you ok?",
  "are you alright",
  ...
  "I like it here",
  "busy night?",
  "what's the story with the lids",
  "who is the supplier",
  "what's the special",
  "tell me about the deep sea prom",
  "when's the prom",
  "can I come to the prom",
  "what's this place called",
  "how long has the bar been here",
  "do you own the bar",
  "who owns this place",
  "what's next door",
  "what's in the shop",
...
const shopProbes = [
  ...
  "ask vesper about the photo",
  "ask vesper about the photograph",
  "ask vesper about sable",
  "vesper, who is this",
  "buy the photo",
  "how much is the photograph",
  "how much",
  "can I buy this",
  "take photo",
  "take the photograph",
  ...
  "can I borrow this",
  "can I take this",
  "vesper, can I take the photograph",
  "show vesper the photo",
...
it("measures vocabulary coverage per state", () => {
  ...
  const fire = (label: string, base: TrialState, probes: string[]) => {
    for (const p of probes) {
      ...
      rows.push(
        `${label}\t${d?.outcome ?? "?"}\t${d?.intent ?? "?"}\t${p}\t${line}`,
      );
    }
  };
  for (const [label, path] of Object.entries(states)) {
    ...
    if (label === "bar_after_show" || label === "booth")
      fire(label, base, showProbes);
  }
  ...
  const lines = Object.entries(tally).map(
    ([k, v]) =>
      `${k.padEnd(22)} ${String(v.ok).padStart(3)}/${v.n}  ${Math.round((100 * v.ok) / v.n)}%`,
  );
  console.log(
